...
export default function BuyerAuth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSuccess("");
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/buyer/login`, {
        method: "POST",
  ...
  return (
    <>
      <Head>
        <title>Buyer Login | Markets</title>
      </Head>
      <div className={styles["auth-container"]}>
        <form className={styles["auth-form"]} onSubmit={handleSubmit}>
          <h2 className={styles["auth-title"]}>Buyer Login</h2>
          <input
            type="email"
            placeholder="Email"
            value={email}
          ...
          <div className={styles["auth-footer"]}>
            ...
            <Link href="/auth/buyer/register">Register</Link>
          </div>
